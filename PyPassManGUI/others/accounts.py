...
def add_acc_to_folder(user, label, folder, acc_username, acc_password):
    def get_digit(accounts_json):
        try:
            last_digit = crypter.decode(list(accounts_json.items())[-1][0])
    ...
    if file_name not in str(os.listdir()):
        ...
        if label != '0':
            ...
            accounts[crypter.encode(str(digit) + digit_separator + acc_username)] = crypter.encode(
            ...
            accounts[crypter.encode(str(digit) + digit_separator + acc_username)] = crypter.encode(acc_password)
...
def get_acc_in_folder(user, folder):
    ...
    if file_name not in str(os.listdir()) or file_name == user + file_extension:
        ...
        for username, password in accounts.items():
            if len(crypter.decode(password).split(label_separator)) == 1:
                digit = crypter.decode(username).split(digit_separator)[0]
                username = crypter.decode(username).split(digit_separator)[-1]
                accounts_list += digit + '.\nusername: ' + username + '\n' + 'password: ' \
                    + crypter.decode(password) + '\n\n'

            else:
                label = crypter.decode(password).split(label_separator)[0]
                password = crypter.decode(password).split(label_separator)[-1]
                digit = crypter.decode(username).split(digit_separator)[0]
                username = crypter.decode(username).split(digit_separator)[-1]
...
def remove_acc_in_folder(user, folder, number, account):
    ...
    if file_name not in str(os.listdir()) or file_name == user + file_extension:
        ...
        account = crypter.encode(number + digit_separator + account)
        ...
        if account not in accounts:
            ...
            new_accounts = {}
            for username, password in accounts.items():
                current_username = crypter.decode(username).split(digit_separator)[1]
                if digit == 1:
...
def backup(user):
    ...
    for folder in folder_list:
        ...
        for username, password in temp.items():
            if len(crypter.decode(password).split(label_separator)) == 1:
                number = crypter.decode(username).split(digit_separator)[0]
                username = crypter.decode(username).split(digit_separator)[1]
    ...
    for full_acc in each_acc_content:
        ...
        for item in new_usernames:
            if new_numbers[y] in acc_numbers_for_label:
                accounts[crypter.encode(new_numbers[y] + digit_separator + item)] = crypter.encode(
                    new_labels[z] + label_separator + new_passwords[x])
                x += 1
                y += 1
                z += 1
            else:
                accounts[crypter.encode(new_numbers[y] + digit_separator + item)] = crypter.encode(new_passwords[x])
                x += 1
                y += 1
